...
<footer class="text-[#b0b0b0]">
	<?php if (is_singular('service') || is_singular('project') || is_singular('post')): ?>
		<?php if (have_rows('ns_post_contact', 'option')): ?>
			<div class="relative max2lg:bg-scroll pt-13.5 pb-5 bg-primary wow fadeIn animated" data-wow-duration="2s"
				data-wow-delay="0.6s" style="visibility: visible; animation-duration: 2s; animation-delay: 0.6s;">
				<div class="container">
					<div class="grid grid-cols-12 gap-x-6">
						<?php while (have_rows('ns_post_contact', 'option')):
							...
							<div class="lg:col-span-3 sm:col-span-6 col-span-12 mb-7.5">
								<div class="relative bg-white overflow-hidden border border-white/15 p-5 rounded group h-full">
									<div>
										<h5 class="text-black font-bold text-lg mb-2">
											<span
												class="-bottom-2 opacity-15 absolute right-0 w-auto z-[2] pointer-events-none duration-700 group-hover:[transform:scale(1.5)_rotate(20deg)]">
		...
		<div
			...
			<div class="container">
				<?php if (have_rows('footer_contact', 'option')): ?>
					<div class="top-footer bg-primary rounded-2lg flex maxmd:flex-wrap maxmd:pb-5  wow fadeIn"
						...
						<?php while (have_rows('footer_contact', 'option')):
							...
							?>
							<div
								class="footer-content md:flex-[0_0_33.33%] md:max-w-[33.33%] relative lg:py-6.25 lg:pr-5 lg:!pl-28 md:p-5 !pl-[75px] z-1 pt-6.25 pr-5 pb-1.25 group after:content-[''] after:h-full after:w-13.5 after:absolute after:-right-3.75 after:top-0 after:-z-1  after:bg-contain after:bg-right after:bg-no-repeat after:opacity-50 maxmd:after:hidden">


								<img src="<?php echo get_stylesheet_directory_uri() ?>/assets/images/<?= $icon ?>" alt="icon"
